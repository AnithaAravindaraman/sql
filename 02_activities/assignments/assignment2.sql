...
-- grouping sales values together
...
--create a table with best and worst day
DROP TABLE IF EXISTS sales_ranked;
...
--query both ranks and bind them together
...
--how many products do we actually have in the vendor inventory?
...
SELECT 
    ...
    SUM(5 * vendor_inventory.quantity * vendor_inventory.original_price) AS total_revenue
FROM 
    vendor_inventory
CROSS JOIN 
    (SELECT DISTINCT customer_purchases.customer_id FROM customer_purchases) AS customer
INNER JOIN 
...
VALUES 
(7, 'apple pie', '10"', '3', 'unit', CURRENT_TIMESTAMP);
